...
for idx, line in district_data.iterrows():
	districts.append(District(idx - 1, int(line['samples'] * FACTOR), Point(float(line['lat']), float(line['lon']) ) ))

fc = open("Centroidsv0.txt", "r");
...
def tryModel():
	...
	for i in range(n):
		...
		model.addConstr( sum(z[i][c] for c in centroidsNearDistrict[i] ) == 1)
		cnt += 1 
	...
	sys.stdout = open("checkOutput" + str(RADIUS) + ".out", "w")
	...
	for i in range(n):
		...
		for j in centroidsNearDistrict[i]:
			if p[i][j].X > 0 :
				sent += p[i][j].X
				# print(0, i+1, j+1, int(p[i][j].X))

		backlog = districts[i].demand - sent 
		if backlog > 0:
			# print(1, i+1, i+1, int(backlog))
...
# to pipe the output 
# print( geodesic((labs[9].location.x, labs[9].location.y), (labs[29].location.x, labs[29].location.y)).km)
tryModel()